...
module {
  ...
  public func _sha256(data : [Nat8]) : [Nat8] {
    let digest = Digest();
    digest.write(data);
    return digest.sum();
  };

  public func sha256(text : Text) : Text {
    let nat8Arr : [Nat8] = Blob.toArray(Text.encodeUtf8(text));
    let sha : [Nat8] = _sha256(nat8Arr);
    let textSha : Text = Hex.encode(sha);
    return textSha;
  };
