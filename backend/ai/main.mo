...
actor {
	...
	public func planProject(prompt : Text) : async ?TypAi.ResponseProjectLLM {
		...
		let tools = [
			LLM.tool("project_planner")
				.withParameter(
					LLM.parameter("project_name", #String)
						.withDescription("Project name. Simple and short but represents the entire description.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("project_tags", #String)
						.withEnumValues(projectTags)
						.withDescription("A tag that represents the project's needs, inferred from the project name. It is used to categorize and suggest relevant tasks or team members. Each tag separated by |.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("task_title", #String)
						.withDescription("Titles of task. Simple and short. Each title separated by |.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("task_description", #String)
						.withDescription("Descriptions of task. Each description separated by |.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("task_tag", #String)
						.withDescription("A task tag used to match and assign the appropriate user.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("task_due_date", #String)
						.withDescription("Format epoch times in second. Each time separated by |.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("task_priority", #String)
						.withEnumValues(["true", "false"])
						.withDescription("Priority of task based on title.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("timeline_title", #String)
						.withDescription("Name of timeline.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("timeline_start", #String)
						.withDescription("Start time of timeline.")
						.isRequired()
				)
				.withParameter(
					LLM.parameter("timeline_end", #String)
						.withDescription("End time of timeline.")
						.isRequired()
				)
				.build()
		];
	...
	public func chat(messages : [LLM.ChatMessage], task : ?Text) : async Text {
		var allMessages = switch(task) {
			case(null) { messages };
			case(?task) {
				let withTaskAssist : LLM.ChatMessage = #user({ content = Prompt.getTaskAssistPrompt(task) });
